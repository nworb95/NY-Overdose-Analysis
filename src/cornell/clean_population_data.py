...
from src.cornell.constants import CORNELL_PROJECTED_DROP_COLUMNS, CORNELL_HISTORICAL_POPULATION_COLUMNS, \
    CORNELL_PROJECTED_POPULATION_COLUMNS, CORNELL_MISSING_POPULATION_YEARS
...
def clean_historical_population_data(df: pd.DataFrame):
    ...
    """
    df = df.rename(
        columns=CORNELL_HISTORICAL_POPULATION_COLUMNS
...
def clean_projected_population_data(df: pd.DataFrame):
    ...
    clean_df = filtered_df.rename(
        columns=CORNELL_PROJECTED_POPULATION_COLUMNS
    ).drop(
...
def merge_population_data(historical_df: pd.DataFrame, projected_df: pd.DataFrame):
    ...
    big_df = pd.concat([df, pd.DataFrame(columns=CORNELL_MISSING_POPULATION_YEARS)], sort=False)
